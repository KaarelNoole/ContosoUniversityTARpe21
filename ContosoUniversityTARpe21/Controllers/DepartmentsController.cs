...
public class DepartmentsController : Controller
    ...
    public async Task<IActionResult> Details(int? id)
    {
        ...
        string quary = "SELECT * FROM Department WHERE DepartmentID = {0}";
    ...
    public async Task<IActionResult> Create([Bind("DepartmentID,Name,StartDate,RowVersion,InstructorID")] Department department)
    {
        ...
        if (await TryUpdateModelAsync<Department>(departmentToUpdate, "", s => s.Name,s => s.StartDate, s => s.Budget,s => s.InstructorID))
        {
            ...
            catch (DbUpdateConcurrencyException ex)
            {
                ...
                else
                {
                    ...
                    {
                        ModelState.AddModelError("Name", $"Current value: {databaseValues.Budget}");
                    }
    ...
    //get delete
    public async Task<IActionResult> Delete(int? id, bool? concurrenctError)
    {
        if (id == null)
        {
            return NotFound();
        ...
        if (department == null)
        {
            if (concurrenctError.GetValueOrDefault())
            {
                return RedirectToAction(nameof(Index));
            }
            return NotFound();
    ...
    public async Task<IActionResult> DeleteConfirmed(Department department)
    {
        ...
        {
            return RedirectToAction(nameof(Delete), new {concurrenyError = true, id = department.DepartmentID});
        }
    }
}
